...
function getNextRow(){
	offset = offset+9;
	$.get('/portfolio/getimages', {'gallery':gallery, 'offset':offset}, function(xml){
		if ($("response image", xml).size() > 0){
			expandSlider();
			$("response image", xml).each(function(){
				$(".portfolio_thumbs .thumb_slider").append('<a href="/portfolio/'+seo+'/'+$(this).attr('id')+'" class="image" id="img_'+$(this).attr('id')+'"><img src="'+$(this).attr('thumb')+'" width="75" height="50" /></a>');
